    ...
    def __init__(self, name, args, data_batch,switch):
        ...
        if args.class_incremental == 'no':
            ...
            if args.positive_percent == 0.5:
                ...
                val_size = int(train_size/4) * args.sites
                if name == 'train':
                ...
                positive_val_switch = int(positive_train_switch/4) * 4
                negative_val_switch = int(negative_train_switch/4) * 4
            ...
            for j in range(data_batch,0,-4):    
                ...
                if name == "train":
                    ...
                    data += total[3-rounds_completed][sites_completed*(train_size+val_size):(sites_completed)*(train_size+val_size)+train_size]
                ...
                if name == "val":
                    ...
                    data += total[3-rounds_completed][sites_completed*(train_size+val_size)+train_size:(sites_completed)*(train_size+val_size)+train_size+val_size]


            
            if name == "test":
                data = list(pd.read_csv(args.data_dir + "/total_test.csv", header=None)[0])
                data = label_0[len(label_4)*(j-1):len(label_4)*j] 
                for i in range(j):
                    data += total[3-i][0:len(label_4)]
            if name == "test_final_loader":
                data = label_0[len(label_4)*(args.rounds):len(label_4)*(args.rounds+1)] 
